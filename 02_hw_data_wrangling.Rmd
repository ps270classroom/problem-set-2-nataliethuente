...
On average, Democratic incumbents promoted the most ads.
...
library(ggplot2)
plot_q3 <- ggplot(fb_ads_us_house, aes(x = impressions_millions)) +
  geom_histogram() 
plot_q3
```
The shape of the histogram trends downward as impressions increases, meaning most candidates had lower amounts of Facebook ads, most US House candidates had less than 10 million ads on Facebook.
...
```{r}
fb_top_plot <- fb_ads_us_house |>
  arrange(desc(impressions_millions)) |>
  head(15) |>
  ggplot(aes(x = impressions_millions, y = reorder(cand_name_last, impressions_millions), fill = party)) + 
  geom_bar(stat = "identity") + 
  labs(title = "Top 15 House Candidates with the most Facebook Ad Impressions",
